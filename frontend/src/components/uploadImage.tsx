...
// const UploadFotoPersilBox: React.FC<UploadFotoPersilBoxProps> = ({ spopData, setSpopData, files, setFiles }) => {
...
//       const newFiles = acceptedFiles.slice(0, 2 - files.length); // antisipasi over-limit
...
//               <Image src={URL.createObjectURL(file)} alt={`Preview ${index + 1}`} width={400} height={300} style={{ borderRadius: 8, objectFit: "cover" }} />
//               <IconButton
//                 onClick={() => {
...
//       {(spopData.foto_op?.length > 0 ? spopData.foto_op : files).length > 0 && (
//         <Box mt={2} display="flex" flexWrap="wrap" gap={2} justifyContent="center">
//           {(spopData.foto_op?.length > 0 ? spopData.foto_op : files).map((item: any, index: number) => (
//             <Box key={index} position="relative">
...
//               <IconButton
//                 onClick={() => {
//                   if (spopData.foto_op?.length > 0) {
//                     // Hapus dari spopData.foto_op
//                     setSpopData((prev: any) => ({
//                       ...prev,
//                       foto_op: prev.foto_op.filter((_: any, i: number) => i !== index),
//                     }));
//                     toast.success("Foto (URL) dihapus spop");
//                   } else {
//                     // Hapus dari files
//                     setFiles((prev: File[]) => prev.filter((_: any, i: number) => i !== index));
//                     toast.success("Foto (File) dihapus files");
//                   }
//                 }}
//                 sx={{ position: "absolute", top: 8, right: 8, backgroundColor: "#fff" }}
//               >
